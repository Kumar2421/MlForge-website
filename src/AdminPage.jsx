...
function AdminPage({ session, navigate }) {
    ...
    const [subscriptionsStatusFilter, setSubscriptionsStatusFilter] = useState('')
    const [subscriptionsPlanFilter, setSubscriptionsPlanFilter] = useState('')

    const adminUserForNav = {
        name:
    ...
    const [inboxMode, setInboxMode] = useState('access')
    ...
    const filteredSubscriptions = (() => {
        const q = String(subscriptionsQuery || '').trim().toLowerCase()
        const status = String(subscriptionsStatusFilter || '').trim().toLowerCase()
        const plan = String(subscriptionsPlanFilter || '').trim().toLowerCase()

        return (subscriptions || []).filter((sub) => {
            if (status && String(sub?.status || '').toLowerCase() !== status) return false
            if (plan && String(sub?.plan_type || '').toLowerCase() !== plan) return false
            if (!q) return true

            const email = users.find((u) => u.user_id === sub.user_id)?.users?.email || ''
            const hay = [email, sub?.user_id, sub?.subscription_id, sub?.razorpay_subscription_id]
                .filter(Boolean)
                .join(' ')
                .toLowerCase()
    ...
    useEffect(() => {
        ...
        const loadSupport = async () => {
            ...
            try {
                const { data, error } = await supabase
                    ...
                    .select('id,user_id,role,message,attachments,created_at')
                    .order('created_at', { ascending: false })
                    .limit(500)
                ...
                rows.forEach((m) => {
                    ...
                    const curT = m?.created_at ? new Date(m.created_at).getTime() : 0
                    if (curT > prevT) byUser.set(uid, m)
                })

                const threads = [...byUser.entries()]
                    .map(([uid, last]) => ({ user_id: uid, last }))
                    .sort((a, b) => {
                ...
                const initialUser = supportSelectedUserId || threads?.[0]?.user_id || ''
                setSupportSelectedUserId(initialUser)
                setSupportMessages(
                    initialUser
                        ? rows
                            .filter((m) => String(m?.user_id || '') === String(initialUser))
                            .sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime())
                        : []
                )
            } catch (e) {
                console.error('Failed to load support inbox:', e)
                toast.error('Failed to load support inbox: ' + (e?.message || String(e)))
            } finally {
        ...
        loadSupport()
    }, [activeTab, inboxMode, isUserAdmin, supportSelectedUserId, toast])
            ...
            .on(
                ...
                (payload) => {
                    ...
                    setSupportThreads((prev) => {
                        const copy = [...prev]
                        const idx = copy.findIndex((t) => String(t.user_id) === uid)
                        const nextThread = { user_id: uid, last: next }
                        if (idx >= 0) copy.splice(idx, 1)
                        copy.unshift(nextThread)
                        return copy
                    })

                    if (String(supportSelectedUserId || '') === uid) {
    ...
    }, [activeTab, inboxMode, isUserAdmin, supportSelectedUserId])

    const sendSupportReply = async () => {
        const text = String(supportDraft || '').trim()
        const targetUserId = String(supportSelectedUserId || '').trim()
        const files = Array.isArray(supportFiles) ? supportFiles : []
        if ((!text && files.length === 0) || !targetUserId) return
        ...
        try {
            ...
            const { error } = await supabase
                .from('support_messages')
                .insert({ user_id: targetUserId, role: 'support', message: text || '', attachments })

            if (error) throw error
            setSupportDraft('')
            setSupportFiles([])
            await adminActionsApi.logAction('support_reply_sent', targetUserId, { message_len: text.length })
        } catch (e) {
            console.error('Failed to send support reply:', e)
            toast.error('Failed to send reply: ' + (e?.message || String(e)))
        } finally {
    ...
    return (
        <SidebarProvider
            ...
            className="dark h-svh w-full overflow-hidden bg-background text-foreground antialiased"
            style={{
                "--sidebar-width": "15rem",
                "--sidebar-width-mobile": "18rem",
            }}
        >
            <Sidebar variant="sidebar" collapsible="none" className="shrink-0">
                <SidebarHeader>
                    <SidebarMenu>
                        <SidebarMenuItem>
                ...
                <SidebarFooter>
                    <NavUser user={adminUserForNav} onNavigate={navigate} onSignOut={handleLogout} />
                </SidebarFooter>
            </Sidebar>
                    ...
                    <div className="flex h-14 items-center justify-between gap-3 px-3 sm:px-6">
                        <div className="flex min-w-0 items-center gap-2">
                            ...
                            <div className="min-w-0 leading-tight">
                                <div className="truncate text-sm font-semibold tracking-tight">Admin Panel</div>
                                <div className="truncate text-xs text-muted-foreground">Platform administration</div>
                            </div>
                        </div>
                        ...
                        <div className="flex w-full max-w-md items-center gap-2">
                            <input
                                ...
                                placeholder="Search user (email or user_id)ΓÇª"
                ...
                >
                    <div className="w-full space-y-4">
                        <PageHeader
                            ...
                            description={
                                activeTab === 'overview'
                                    ? 'Platform health, revenue, and recent issues'
                                    : activeTab === 'users'
                                                        ...
                                                        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                                                            <Card>
                                                                <CardHeader className="pb-2">
                                                                    ...
                                                                    <CardTitle className="text-base">{user.email || 'ΓÇö'}</CardTitle>
                                                                </CardHeader>
                                                            </Card>
                                                            <Card>
                                                                <CardHeader className="pb-2">
                                                                    <CardDescription>User id</CardDescription>
                                                                    <CardTitle className="text-base">{String(selectedUserId).substring(0, 12)}ΓÇª</CardTitle>
                                                                </CardHeader>
                                                            </Card>
                                                            <Card>
                                                                <CardHeader className="pb-2">
                                                            ...
                                                            <CardContent className="space-y-3">
                                                                <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
                                                                    <div className="flex flex-1 flex-col gap-2 sm:flex-row sm:items-center">
                                                                        ...
                                                                        >
                                                                            <option value="">Assign licenseΓÇª</option>
                                                                                    ...
                                                                                    <tbody>
                                                                                        {userDetailEvents.slice(0, 20).map((evt) => (
                                                                                            <tr key={String(evt.id)} className="border-b last:border-b-0">
                                                                                                ...
                                                                                                <td className="px-3 py-2">{evt.event_type || 'ΓÇö'}</td>
                                                                                                <td className="px-3 py-2">{evt.kind || 'ΓÇö'}</td>
                                                                                                <td className="px-3 py-2">{evt.app_version || 'ΓÇö'}</td>
                                                                                                <td className="px-3 py-2 max-w-[420px] truncate">{evt.message || 'ΓÇö'}</td>
                                                                                            </tr>
                                                                                        ))}
                                                                                    </tbody>
                                                                                </table>
                                                                                    ...
                                                                                    <tbody>
                                                                                        {userDetailActions.slice(0, 20).map((action) => (
                                                                                            <tr key={String(action.action_id)} className="border-b last:border-b-0">
                                                                                                ...
                                                                                                <td className="px-3 py-2 max-w-[420px] truncate">{action.details ? JSON.stringify(action.details) : 'ΓÇö'}</td>
                                                                                            </tr>
                                                                                        ))}
                                                                                    </tbody>
                                                                                </table>
                                                                            ...
                                                                            <tbody>
                                                                                {userDetailSubscriptions.slice(0, 10).map((sub) => (
                                                                                    <tr key={String(sub.subscription_id || sub.id)} className="border-b last:border-b-0">
                                                                                        ...
                                                                                        <td className="px-3 py-2">{sub.status || 'ΓÇö'}</td>
                                                                                        <td className="px-3 py-2">{sub.plan_type || sub.plan || 'ΓÇö'}</td>
                                            ...
                                            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                                                ...
                                                <Card>
                                                    <CardHeader className="pb-2">
                                                        ...
                                                        <CardTitle className="text-2xl">
                                                            {systemHealth?.syncHealth?.successRate != null ? `${systemHealth.syncHealth.successRate}%` : 'ΓÇö'}
                                                        </CardTitle>
                                                    </CardHeader>
                                                </Card>
                                                <Card>
                                                    <CardHeader className="pb-2">
                                                        ...
                                                        <CardTitle className="text-2xl">
                                                            {revenueAnalyticsEnhanced?.mrr != null
                                                                ? `Γé╣${parseFloat(revenueAnalyticsEnhanced.mrr).toLocaleString('en-IN')}`
                                                                : 'ΓÇö'}
                                                        </CardTitle>
                                                    </CardHeader>
                                                </Card>
                                                <Card>
                                                    <CardHeader className="pb-2">
                                                        ...
                                                        <CardTitle className="text-2xl">{systemHealth?.activeSessions ?? 'ΓÇö'}</CardTitle>
                                                    </CardHeader>
                                                </Card>
                                            </div>
                                        )}
                                                    ...
                                                    <div className="divide-y rounded-md border">
                                                        {expiring.map((u) => (
                                                            <div key={String(u.userId)} className="flex items-center justify-between gap-3 px-3 py-3 text-sm">
                                                                <div className="min-w-0">
                                                                    <div className="truncate font-medium">{u.email || String(u.userId || '').substring(0, 8) + 'ΓÇª'}</div>
                                                                    <div className="text-xs text-muted-foreground">{String(u.userId || '').substring(0, 8)}ΓÇª</div>
                                                                </div>
                                                    ...
                                                    {overviewRecentCrashes.map((evt) => (
                                                        <div key={String(evt.id)} className="px-3 py-3 text-sm">
                                                            ...
                                                            <div className="mt-1 text-xs text-muted-foreground">
                                                                {(evt.user_id || '').substring(0, 8)}ΓÇª ┬╖ {evt.platform || 'ΓÇö'} ┬╖ {evt.app_version || 'ΓÇö'}
                                                            </div>
                                                            <div className="mt-1 max-w-[720px] truncate text-sm text-muted-foreground">{evt.message || 'ΓÇö'}</div>
                                                        </div>
                                                    ))}
                                                </div>
                                            )}
                                                    ...
                                                    <table className="w-full text-sm">
                                                        ...
                                                        <tbody>
                                                            {securityBlocklist.slice(0, 30).map((row) => {
                                                                const target = row.email || row.ip_address || row.user_id || 'ΓÇö'
                                                                return (
                                                                    <tr key={String(row.block_id)} className="border-b last:border-b-0">
                                                                        <td className="px-3 py-2">{String(target)}</td>
                                                                        <td className="px-3 py-2 max-w-[360px] truncate">{row.reason || 'ΓÇö'}</td>
                                                    ...
                                                    <table className="w-full text-sm">
                                                        ...
                                                        <tbody>
                                                            {securityEvents.slice(0, 30).map((evt) => {
                                                                const target = evt.email || evt.ip_address || evt.user_id || 'ΓÇö'
                                                                return (
                                ...
                                <CardContent className="space-y-3">
                                    <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                                        <input
                                            ...
                                            placeholder="Search message/platform/user/app_versionΓÇª"
                                            className="h-9 rounded-md border bg-background px-3 text-sm"
                                        />
                                        <select
                                            className="h-9 rounded-md border bg-background px-3 text-sm"
                                                ...
                                                <tbody>
                                                    {diagnosticsEvents.length === 0 ? (
                                                        ...
                                                        diagnosticsEvents.map((evt) => (
                                                            <tr key={String(evt.id)} className="border-b last:border-b-0">
                                                                <td className="px-3 py-2 whitespace-nowrap">{formatDateTime(evt.created_at)}</td>
                                                                <td className="px-3 py-2 whitespace-nowrap">{(evt.user_id || '').substring(0, 8)}ΓÇª</td>
                                                                <td className="px-3 py-2">{evt.event_type || 'ΓÇö'}</td>
                                                                <td className="px-3 py-2">{evt.kind || 'ΓÇö'}</td>
                                                                <td className="px-3 py-2">{evt.platform || 'ΓÇö'}</td>
                                                                <td className="px-3 py-2">{evt.app_version || 'ΓÇö'}</td>
                                                                <td className="px-3 py-2 max-w-[520px] truncate">{evt.message || 'ΓÇö'}</td>
                                                            </tr>
                                                        ))
                                                    )}
                                                </tbody>
                                        ...
                                        <CardContent className="space-y-3">
                                            <div className="grid gap-2 sm:grid-cols-2">
                                                <input
                                                    ...
                                                    value={diagnosticsQuery}
                                                    onChange={(e) => setDiagnosticsQuery(e.target.value)}
                                                    placeholder="Search message/platform/app_version/kindΓÇª"
                                                    className="h-9 rounded-md border bg-background px-3 text-sm"
                                                />
                                                <input
                                                    type="text"
                                            ...
                                            <div className="grid gap-2 sm:grid-cols-2">
                                                <input
                                                    ...
                                                    value={diagnosticsKindFilter}
                                                    onChange={(e) => setDiagnosticsKindFilter(e.target.value)}
                                                    placeholder="Filter kind (main/renderer/renderer-goneΓÇª)"
                                                    className="h-9 rounded-md border bg-background px-3 text-sm"
                                                />
                                                <select
                                                    className="h-9 rounded-md border bg-background px-3 text-sm"
                                                                ...
                                                                <div className="flex items-start justify-between gap-3">
                                                                    <div className="min-w-0">
                                                                        <div className="font-medium">
                                                                            {sig.kind || 'ΓÇö'}{sig.source ? ` ┬╖ ${sig.source}` : ''}
                                                                        </div>
                                                                        <div className="mt-1 max-w-[520px] truncate text-xs text-muted-foreground">{sig.message || 'ΓÇö'}</div>
                                                                    </div>
                                            ...
                                            <div className="space-y-4">
                                                <div className="grid gap-2 sm:grid-cols-2">
                                                    ...
                                                    <div className="rounded-md border p-3">
                                                        ...
                                                        <div className="mt-1 text-sm">{(selectedDiagnosticsEvent.user_id || '').substring(0, 8)}ΓÇª</div>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="grid gap-2 sm:grid-cols-2">
                                                    <div className="rounded-md border p-3">
                                                        ...
                                                        <div className="mt-1 text-sm">{selectedDiagnosticsEvent.kind || 'ΓÇö'}</div>
                                                    </div>
                                                    <div className="rounded-md border p-3">
                                                        <div className="text-xs font-medium text-muted-foreground">Source</div>
                                                        <div className="mt-1 text-sm">{selectedDiagnosticsEvent.source || 'ΓÇö'}</div>
                                                    </div>
                                                </div>

                                                <div className="rounded-md border p-3">
                                                    <div className="text-xs font-medium text-muted-foreground">Message</div>
                                                    <div className="mt-1 text-sm whitespace-pre-wrap">{selectedDiagnosticsEvent.message || 'ΓÇö'}</div>
                                                </div>

                                                <div className="rounded-md border p-3">
                                                    <div className="text-xs font-medium text-muted-foreground">Stack</div>
                                                    <pre className="mt-2 max-h-[360px] overflow-auto whitespace-pre-wrap text-xs">{selectedDiagnosticsEvent.stack || 'ΓÇö'}</pre>
                                                </div>

                                                <div className="rounded-md border p-3">
                                                    <div className="text-xs font-medium text-muted-foreground">Payload</div>
                                                    <pre className="mt-2 max-h-[360px] overflow-auto whitespace-pre-wrap text-xs">{selectedDiagnosticsEvent.payload ? JSON.stringify(selectedDiagnosticsEvent.payload, null, 2) : 'ΓÇö'}</pre>
                                                </div>
                                            </div>
                                        )}
                                    </CardContent>
                        ...
                        {/* Licenses Tab */}
                        {activeTab === 'licenses' && (
                            <SectionCard title="Licenses" description="Assign plans, expiry, offline flags and tokens">
                                <div className="overflow-auto rounded-md border">
                                    <table className="w-full text-sm">
                                        <thead className="bg-muted/30 text-xs text-muted-foreground">
                                            <tr className="border-b">
                        ...
                        {activeTab === 'subscriptions' && (
                            <div className="space-y-4">
                                <SectionCard title="Subscriptions" description="Active subscriptions">
                                    <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                        <div className="flex flex-1 items-center gap-2">
                                            <input
                                                value={subscriptionsQuery}
                                                onChange={(e) => setSubscriptionsQuery(e.target.value)}
                                                placeholder="Search by email, user_id, or subscriptionΓÇª"
                                                className="h-9 w-full rounded-md border bg-background px-3 text-sm"
                                            />
                                        </div>
                                        <div className="flex flex-wrap items-center gap-2">
                                            ...
                                            <tbody>
                                                {filteredSubscriptions.length === 0 ? (
                                                    ...
                                                    filteredSubscriptions.map((sub) => (
                                                        <tr key={String(sub.subscription_id)} className="border-b last:border-b-0 hover:bg-muted/30">
                                                            ...
                                                            <td className="px-3 py-2 font-mono text-xs">
                                                                {sub.razorpay_subscription_id ? String(sub.razorpay_subscription_id).substring(0, 20) + 'ΓÇª' : 'ΓÇö'}
                                                            </td>
                                                        </tr>
                                                    ))
                                                )}
                                ...
                                <SectionCard title="Recent payments" description="Latest 50 billing events">
                                    <div className="overflow-hidden rounded-lg border">
                                        <table className="w-full text-sm">
                                            <thead className="bg-muted/30 text-xs text-muted-foreground">
                                                <tr className="border-b">
                                            ...
                                            <tbody>
                                                {billingHistory.length === 0 ? (
                                                    ...
                                                    billingHistory.map((payment) => (
                                                        <tr key={String(payment.billing_id)} className="border-b last:border-b-0 hover:bg-muted/30">
                                                            <td className="px-3 py-2">{formatDate(payment.created_at)}</td>
                                                            <td className="px-3 py-2">{payment.user_id?.substring(0, 8)}</td>
                                                            <td className="px-3 py-2">Γé╣{(Number(payment.amount || 0) / 100).toFixed(2)}</td>
                                                            <td className="px-3 py-2">{payment.status || 'ΓÇö'}</td>
                                                            <td className="px-3 py-2 font-mono text-xs">
                                                                {payment.razorpay_payment_id ? String(payment.razorpay_payment_id).substring(0, 20) + 'ΓÇª' : 'ΓÇö'}
                                                            </td>
                                                        </tr>
                                                    ))
                                                )}
                                ...
                                <CardHeader>
                                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                        <div>
                                            ...
                                            <CardDescription>Access requests and support conversations</CardDescription>
                                        </div>
                                        <div className="text-xs text-muted-foreground">
                                            {inboxMode === 'support' ? `Threads: ${supportThreads.length}` : `Total: ${inboxMessages.length}`}
                                        </div>
                                    </div>
                                </CardHeader>
                                <CardContent className="space-y-3">
                                    <div className="flex flex-wrap gap-2">
                                        <Button
                                            type="button"
                                            variant={inboxMode === 'access' ? 'default' : 'outline'}
                                            size="sm"
                                            onClick={() => setInboxMode('access')}
                                        >
                                            Access requests
                                        </Button>
                                        <Button
                                            type="button"
                                            variant={inboxMode === 'support' ? 'default' : 'outline'}
                                            size="sm"
                                            onClick={() => setInboxMode('support')}
                                        >
                                            Support
                                        </Button>
                                    </div>
                                    ...
                                    {inboxMode === 'support' ? (
                                        <div className="grid gap-3 lg:grid-cols-[320px_1fr]">
                                            <Card>
                                                ...
                                                <CardContent className="p-0">
                                                    {supportLoading ? (
                                                        ...
                                                        <div className="px-4 py-6 text-sm text-muted-foreground">No support messages yet.</div>
                                                    ) : (
                                                        <div className="max-h-[520px] overflow-y-auto" style={{ scrollbarGutter: 'stable' }}>
                                                            {supportThreads.map((t) => {
                                                                const uid = String(t.user_id || '')
                                                                const isActive = uid && uid === String(supportSelectedUserId || '')
                                                                const preview = t?.last?.message || ''
                                                                return (
                                                                    <button
                                                                        key={uid}
                                                                        type="button"
                                                                        className={
                                                                            isActive
                                                                                ? 'w-full border-b bg-muted/30 px-4 py-3 text-left'
                                                                                : 'w-full border-b px-4 py-3 text-left hover:bg-muted/20'
                                                                        }
                                                                        onClick={() => {
                                                                            setSupportSelectedUserId(uid)
                                                                            const rows = (supportThreads || [])
                                                                                .flatMap((x) => [])
                                                                            void rows
                                                                        }}
                                                                    >
                                                                        <div className="flex items-start justify-between gap-2">
                                                                            <div className="min-w-0">
                                                                                <div className="truncate text-sm font-semibold">{uid.substring(0, 8)}ΓÇª</div>
                                                                                <div className="truncate text-xs text-muted-foreground">{preview}</div>
                                                                            </div>
                                                                            <div className="shrink-0 text-[10px] text-muted-foreground">
                                                                                {t?.last?.created_at ? formatDateTime(t.last.created_at) : ''}
                                                                            </div>
                                                                        </div>
                                                                    </button>
                                                                )
                                            ...
                                            <Card>
                                                <CardHeader>
                                                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                                        <div>
                                                            ...
                                                            <CardDescription>
                                                                {supportSelectedUserId ? `User: ${supportSelectedUserId.substring(0, 8)}ΓÇª` : 'Select a thread'}
                                                            </CardDescription>
                                                        </div>
                                                        <Button
                                                            type="button"
                                                            variant="outline"
                                                            size="sm"
                                                            disabled={!supportSelectedUserId}
                                                            onClick={() => {
                                                                if (supportSelectedUserId) navigateToUserDetail(supportSelectedUserId)
                                                            }}
                                                        >
                                                            Open user
                                                        </Button>
                                                ...
                                                <CardContent className="space-y-3">
                                                    <div className="max-h-[420px] overflow-y-auto rounded-md border p-3" style={{ scrollbarGutter: 'stable' }}>
                                                        {supportSelectedUserId && supportMessages.length === 0 ? (
                                                            <div className="text-sm text-muted-foreground">No messages in this thread yet.</div>
                                                        ) : !supportSelectedUserId ? (
                                                            <div className="text-sm text-muted-foreground">Pick a thread to start replying.</div>
                                                        ) : (
                                                            <div className="space-y-2">
                                                                {supportMessages.map((m) => {
                                                                    const isSupport = String(m?.role || '') === 'support'
                                                                    const atts = Array.isArray(m?.attachments) ? m.attachments : []
                                                                    return (
                                                                        <div key={String(m?.id || m?.created_at)} className={isSupport ? 'flex justify-end' : 'flex justify-start'}>
                                                                            <div className={isSupport ? 'max-w-[85%] rounded-lg bg-foreground px-3 py-2 text-sm text-background' : 'max-w-[85%] rounded-lg border bg-background px-3 py-2 text-sm'}>
                                                                                ...
                                                                                {atts.length > 0 ? (
                                                                                    <div className="mt-2 space-y-2">
                                                                                        {atts.map((a) => {
                                                                                            ...
                                                                                            return (
                                                                                                <div key={String(a?.path || url || name)} className={isSupport ? 'rounded-md bg-background/10 p-2' : 'rounded-md bg-muted/30 p-2'}>
                                                                                                    {isImage && url ? (
                                                                                                        <a href={url} target="_blank" rel="noreferrer" className="block">
                                                                                                            <img src={url} alt={name} className="max-h-48 w-auto rounded-md border" />
                                                                                                        </a>
                                                                                                    ) : (
                                                                                ...
                                                                                <div className={isSupport ? 'mt-1 text-[10px] opacity-70' : 'mt-1 text-[10px] text-muted-foreground'}>
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <label className="inline-flex h-9 w-9 cursor-pointer items-center justify-center rounded-md border bg-background text-sm hover:bg-muted/30">
                                                            <input
                                                                type="file"
                                                                className="hidden"
                                                                multiple
                                                        ...
                                                        <Button
                                                            ...
                                                            disabled={supportSending || supportUploading || !supportSelectedUserId || (!String(supportDraft || '').trim() && (supportFiles || []).length === 0)}
                                                        >
                                                            Send
                                                        </Button>
                                                    </div>
                                                    {(supportFiles || []).length > 0 ? (
                                                        <div className="text-xs text-muted-foreground">Attachments: {(supportFiles || []).map((f) => f?.name).filter(Boolean).join(', ')}</div>
                                                    ) : null}
                                                </CardContent>
                                            </Card>
                                        </div>
                                                                    ...
                                                                    .map((msg) => (
                                                                        <tr key={String(msg.request_id)} className="border-b last:border-b-0">
                                                                            ...
                                                                            <td className="px-3 py-2 whitespace-nowrap">
                                                                                {msg.linked_user_id ? String(msg.linked_user_id).substring(0, 8) + 'ΓÇª' : String(msg.user_id || '').substring(0, 8) + 'ΓÇª'}
                                                                            </td>
                                                                            <td className="px-3 py-2">
                                                                                <select
                                                                            ...
                                                                            <td className="px-3 py-2 whitespace-nowrap">
                                                                                {msg.crash_event_id ? String(msg.crash_event_id).substring(0, 8) + 'ΓÇª' : 'ΓÇö'}
                                                                            </td>
                                                                            <td className="px-3 py-2 max-w-[520px] truncate">{msg.message || msg.details || 'ΓÇö'}</td>
                                                                            <td className="px-3 py-2 text-right">
                                                                                <div className="inline-flex flex-wrap justify-end gap-2">
                                                                                    <Button
                                                                                        variant="outline"
                                                ...
                                                {auditLog.length === 0 ? (
                                                    ...
                                                    auditLog.map((action) => (
                                                        <tr key={String(action.action_id)} className="border-b last:border-b-0">
                                                            ...
                                                            <td className="px-3 py-2">{getUserEmailById(action.admin_user_id) || 'N/A'}</td>
                                                            <td className="px-3 py-2">{String(action.action_type || '').replace(/_/g, ' ')}</td>
                                                            <td className="px-3 py-2">{getUserEmailById(action.target_user_id) || 'ΓÇö'}</td>
                                                            <td className="px-3 py-2 max-w-[420px] truncate">
                                                                {action.details ? JSON.stringify(action.details) : 'ΓÇö'}
                                                            </td>
                                                        </tr>
                                                    ))
                                                )}
                                                ...
                                                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                                                    ...
                                                    <Card>
                                                        <CardHeader className="pb-2">
                                                            ...
                                                            <CardTitle className="text-2xl">Γé╣{Number(gpuUsage.totals?.totalCost || 0).toFixed(2)}</CardTitle>
                                                        </CardHeader>
                                                        <CardContent className="text-xs text-muted-foreground">All time</CardContent>
                                                    </Card>
                                                    <Card>
                                                    ...
                                                    <Card>
                                                        <CardHeader className="pb-2">
                                                            ...
                                                            <CardTitle className="text-2xl">
                                                                Γé╣{Number(gpuUsage.totals?.count || 0) > 0 ? (Number(gpuUsage.totals?.totalCost || 0) / Number(gpuUsage.totals.count)).toFixed(2) : '0.00'}
                                                            </CardTitle>
                                                        </CardHeader>
                                                        <CardContent />
                                                    </Card>
                                                ...
                                                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                                                    <Card>
                                                        <CardHeader className="pb-2">
                                                            ...
                                                            <CardTitle className="text-2xl">Γé╣{parseFloat(revenueAnalyticsEnhanced.mrr).toLocaleString('en-IN')}</CardTitle>
                                                        </CardHeader>
                                                    </Card>
                                                    <Card>
                                                        <CardHeader className="pb-2">
                                                            <CardDescription>Annual recurring revenue (ARR)</CardDescription>
                                                            <CardTitle className="text-2xl">Γé╣{parseFloat(revenueAnalyticsEnhanced.arr).toLocaleString('en-IN')}</CardTitle>
                                                        </CardHeader>
                                                    </Card>
                                                    <Card>
                                                        <CardHeader className="pb-2">
                                                                ...
                                                                {Object.entries(revenueAnalyticsEnhanced.revenueByPlan).map(([plan, revenue]) => (
                                                                    <div key={plan} className="flex items-start justify-between gap-3 px-3 py-3 text-sm">
                                                                        ...
                                                                        <div className="text-right">
                                                                            <div className="font-medium">Γé╣{((revenue.monthly || 0) + (revenue.yearly || 0) / 12).toFixed(2)}</div>
                                                    ...
                                                    {userActivity.activities && userActivity.activities.length > 0 ? (
                                                        userActivity.activities.slice(0, 50).map((activity) => (
                                                            <div key={activity.activity_id} className="flex items-start justify-between gap-3 px-3 py-3 text-sm">
                                                                ...
                                                                <div className="text-xs text-muted-foreground">{String(activity.user_id || '').substring(0, 8)}ΓÇª</div>
                                                            </div>
                                                        ))
                                                    ) : (

...
export default function DashboardV2Page({ session, navigate, lockedTab = null }) {
    ...
    const dashboardSubTabs = React.useMemo(() => ["overview", "activity", "runs", "models"], [])
    ...
    const [securityEmail, setSecurityEmail] = React.useState(session?.user?.email || "")
    const [securityEmailUpdating, setSecurityEmailUpdating] = React.useState(false)
    const [securityResetSending, setSecurityResetSending] = React.useState(false)
    ...
    const [supportFiles, setSupportFiles] = React.useState([])
    const [supportUploading, setSupportUploading] = React.useState(false)
    ...
    React.useEffect(() => {
        ...
        loadSupportMessages()
    }, [loadSupportMessages, supportOpen])
    ...
    const sendSupportMessage = React.useCallback(async () => {
        ...
        try {
            ...
            const { error } = await supabase
                .from("support_messages")
                .insert({ user_id: userId, role: "user", message: text || "", attachments })
            ...
            toast.error("Failed to send message: " + (e?.message || String(e)))
        } finally {
            setSupportUploading(false)
            setSupportSending(false)
        }
    }, [supportDraft, supportFiles, toast, userId])
    ...
    const shouldShowOnboardingGate = React.useMemo(() => {
        const onboardingIncomplete = !setupChecklist.hasSynced || !setupChecklist.hasProjects
        if (!onboardingIncomplete) return false

        const bypassTabs = ["billing", "settings", "diagnostics"]
        return !bypassTabs.includes(String(activeTab || "overview"))
    }, [activeTab, setupChecklist.hasProjects, setupChecklist.hasSynced])
    ...
    return (
        ...
        >
            ...
            <Sheet open={supportOpen} onOpenChange={setSupportOpen}>
                <SheetContent side="right" className="dark flex w-full flex-col bg-background p-0 text-foreground sm:max-w-md">
                    <SheetHeader className="border-b bg-background px-6 py-5">
                        <SheetTitle>Support</SheetTitle>Chat with the ML Forge team.
                        <SheetDescription></SheetDescription>
                    </SheetHeader>

                    <div className="flex min-h-0 flex-1 flex-col bg-background">
                        <div ref={supportListRef} className="min-h-0 flex-1 overflow-y-auto bg-background px-6 py-4" style={{ scrollbarGutter: "stable" }}>
                            {supportLoading ? (
                                ...
                                <div className="space-y-3">
                                    <div className="text-sm text-muted-foreground">Failed to load support messages: {supportError}</div>
                                    <Button type="button" variant="outline" size="sm" onClick={() => loadSupportMessages()}>
                                        Retry
                                    </Button>
                                </div>
                            ) : supportMessages.length === 0 ? (
                                <div className="text-sm text-muted-foreground">Send a message to start a support thread.</div>
                            ) : (
                                <div className="space-y-3">
                                    {supportMessages.map((m) => {
                                        const isUser = (m?.role || "user") === "user"
                                        const atts = Array.isArray(m?.attachments) ? m.attachments : []
                                        return (
                                            <div key={String(m?.id || m?.created_at)} className={isUser ? "flex justify-end" : "flex justify-start"}>
                                                <div className={isUser ? "max-w-[85%] rounded-lg bg-muted px-3 py-2 text-sm text-foreground" : "max-w-[85%] rounded-lg border bg-background px-3 py-2 text-sm text-foreground"}>
                                                    ...
                                                    {atts.length > 0 ? (
                                                        <div className="mt-2 space-y-2">
                                                            {atts.map((a) => {
                                                                ...
                                                                return (
                                                                    <div key={String(a?.path || url || name)} className={isUser ? "rounded-md bg-background/10 p-2" : "rounded-md bg-muted/30 p-2"}>
                                                                        {isImage && url ? (
                                                    ...
                                                    <div className={isUser ? "mt-1 text-[10px] opacity-70" : "mt-1 text-[10px] text-muted-foreground"}>
                        ...
                        <div className="border-t bg-background px-6 py-4">
                            <div className="flex items-center gap-2">
                                ...
                                <Input
                                    ...
                                    placeholder="Describe your issueΓÇª"
                                    onKeyDown={(e) => {
                                ...
                                />
                                <Button onClick={() => sendSupportMessage()} disabled={supportSending || supportUploading || supportLoading || (!supportDraft.trim() && (supportFiles || []).length === 0)}>
                                    Send
                                </Button>
                            </div>
                            {(supportFiles || []).length > 0 ? (
                                <div className="mt-2 text-xs text-muted-foreground">Attachments: {(supportFiles || []).map((f) => f?.name).filter(Boolean).join(', ')}</div>
                            ) : null}
                            <div className="mt-2 text-xs text-muted-foreground">
                                Tip: include your project name and what you expected to happen.
                            </div>
                        </div>
                    </div>
                </SheetContent>
                    ...
                    <div className="flex h-14 items-center justify-between gap-3 px-3 sm:px-6">
                                ...
                                <div className="flex min-w-0 items-center gap-2">
                                    ...
                                    {selectedProject ? (
                                        <div className="hidden items-center gap-2 text-xs text-muted-foreground sm:flex">
                                            ...
                                            <span className="truncate">{getProjectName(selectedProject)}</span>
                                        </div>
                                    ) : null}
                                </div>
                        ...
                        <div className="flex items-center gap-2">
                            <div className="hidden items-center gap-2 md:flex">
                                <select
                                    className="h-9 max-w-[260px] rounded-md border bg-background px-3 text-sm"
                                    value={selectedProjectId}
                                    onChange={(e) => {
                                        const next = e.target.value
                                        setSelectedProjectId(next)
                                        goToDashboardProject(next, activeTab)
                                    }}
                                >
                    ...
                    {shouldShowOnboardingGate ? (
                        ...
                        <div className="w-full">
                            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                                <TabsList className="mb-4 flex h-10 flex-wrap justify-start gap-1 overflow-hidden rounded-md border border-white/10 bg-black p-1 text-white">
                                    <TabsTrigger
                                        value="overview"
                                        className="text-zinc-200 data-[state=active]:bg-white/10 data-[state=active]:text-white"
                                    >
                                        Dashboard
                                    </TabsTrigger>
                                    <TabsTrigger
                                ...
                                {dashboardSubTabs.includes(String(activeTab || "")) ? (
                                    <div className="mb-4">
                                        <div className="flex h-9 flex-nowrap items-center justify-start gap-1 overflow-x-auto rounded-md border bg-background p-1">
                                            <Button type="button" variant={activeTab === "overview" ? "default" : "ghost"} size="sm" onClick={() => setActiveTab("overview")}>
                                                Overview
                                            </Button>
                                            <Button type="button" variant={activeTab === "activity" ? "default" : "ghost"} size="sm" onClick={() => setActiveTab("activity")}>
                                                Activity
                                            </Button>
                                            <Button type="button" variant={activeTab === "runs" ? "default" : "ghost"} size="sm" onClick={() => setActiveTab("runs")}>
                                                Runs
                                            </Button>
                                            <Button
                                                type="button"
                                ...
                                <TabsContent value="overview" className="space-y-4">
                                    <SectionCard
                                        title="Workspace"
                                        description="Read-only web dashboard. Projects, runs, models, and exports are created in the IDE and synced here."
                                    >
                                    ...
                                    {subscriptionSummary?.plan_type === 'free' && !activeTrial && (
                                        <Card>
                                            <CardHeader>
                                                ...
                                                <CardDescription>Upgrade if youΓÇÖre hitting limits (projects, exports, GPU hours).</CardDescription>
                                            </CardHeader>
                                            <CardContent>
                                    ...
                                    <div className="grid gap-3 lg:grid-cols-2">
                                        <Card>
                                            <CardHeader>
                                                ...
                                                <CardDescription>IDE Γåö dashboard metadata sync</CardDescription>
                                            </CardHeader>
                                        ...
                                        <Card>
                                            ...
                                            <CardContent className="space-y-3">
                                                {loading.timeline ? (
                                                    <div className="text-sm text-muted-foreground">Loading timelineΓÇª</div>
                                                    ...
                                                    projectModels.slice(0, 5).map((m) => (
                                                        <div key={String(m.id || m.model_id || `${m.trained_at || m.updated_at}-${m.name}`)} className="flex items-start justify-between gap-3">
                                                            <div className="min-w-0">
                                                                ...
                                                                <div className="text-xs text-muted-foreground">
                                                                    {m.version || m.model_version ? `v${m.version || m.model_version}` : 'ΓÇö'}
                                    ...
                                    <div className="space-y-4">
                                            ...
                                            <CardContent className="space-y-4">
                                                <div className="grid gap-2 text-sm">
                                                    ...
                                                    <div className="flex items-center justify-between gap-3">
                                                        ...
                                                        <div className="font-medium">{session?.user?.email || 'ΓÇö'}</div>
                                                    </div>
                                                </div>
                                        ...
                                        <Card>
                                            <CardHeader>
                                                ...
                                                <CardDescription>View your current plan and upgrade anytime.</CardDescription>
                                            </CardHeader>
                                            <CardContent className="space-y-3">
                                        ...
                                        <Card>
                                            <CardHeader>
                                                ...
                                                <CardDescription>Update email and password security settings.</CardDescription>
                                            </CardHeader>
                                            <CardContent className="space-y-4">
                                                <div className="grid gap-2">
                                                    <div className="text-sm font-semibold tracking-tight">Update Email</div>
                                                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                                                        <Input
                                                            ...
                                                            className="h-9"
                                                            disabled={securityEmailUpdating || deleteAccountLoading}
                                                        />
                                                        <Button
                                                            variant="outline"
                                                            className="h-9"
                                                            disabled={
                                                                securityEmailUpdating ||
                                                                deleteAccountLoading ||
                                                                !securityEmail.trim() ||
                                                                securityEmail.trim() === String(session?.user?.email || '').trim()
                                                            }
                                                            onClick={async () => {
                                                                const nextEmail = securityEmail.trim()
                                                                if (!nextEmail) return
                                                                setSecurityEmailUpdating(true)
                                                                try {
                                                                    const { error } = await supabase.auth.updateUser({ email: nextEmail })
                                                                    if (error) throw error
                                                                    toast.success('Email update requested. Please confirm via the link sent to your email.')
                                                                } catch (e) {
                                                                    toast.error(e?.message || 'Failed to update email')
                                                                } finally {
                                                                    setSecurityEmailUpdating(false)
                                                                }
                                                            }}
                                                        >
                                                            {securityEmailUpdating ? 'UpdatingΓÇª' : 'Update'}
                                                        </Button>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="grid gap-2">
                                                    ...
                                                    <div className="flex flex-wrap gap-2">
                                                        <Button
                                                            variant="outline"
                                                            disabled={securityResetSending || deleteAccountLoading || !session?.user?.email}
                                                            onClick={async () => {
                                                                if (!session?.user?.email) return
                                                                setSecurityResetSending(true)
                                                                try {
                                                                    ...
                                                                    setSecurityResetSending(false)
                                                                }
                                                            }}
                                                        >
                                                            {securityResetSending ? 'SendingΓÇª' : 'Send reset code'}
                                                        </Button>
                                                    </div>
                                                </div>
                                            </CardContent>
                                            ...
                                            <CardContent className="space-y-3">
                                                ...
                                                <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                                                    ...
                                                    <Button
                                                        variant="destructive"
                                                        className="h-9"
                                                        disabled={deleteAccountLoading || deleteConfirmText.trim() !== 'DELETE'}
                                                        onClick={async () => {
                                                    ...
                                                    >
                                                        {deleteAccountLoading ? 'DeletingΓÇª' : 'Delete account'}
                                                    </Button>
                                                </div>
                                            </CardContent>
                                        </Card>
                                ...
                                <TabsContent value="projects">
                                    <Card>
                                        <CardHeader>
                                            ...
                                            <CardDescription>Select a project to scope runs, models, and exports.</CardDescription>
                                        </CardHeader>
                                        <CardContent className="space-y-3">
                                            {loading.projects ? (
                                                <div className="space-y-2">
                                                ...
                                                <div className="text-sm text-muted-foreground">No projects found yet.</div>
                                            ) : (
                                                <div className="overflow-auto rounded-md border">
                                                    <table className="w-full text-sm">
                                                        <thead className="bg-muted/30 text-xs text-muted-foreground">
                                                        ...
                                                        <tbody>
                                                            {projects.map((p) => {
                                                                const pid = String(getProjectId(p) || "")
                                                                const name = getProjectName(p)
                                                                const isSelected = pid && String(selectedProjectId) === pid
                                                                return (
                                                                    <tr key={pid || name} className="border-b last:border-b-0 hover:bg-muted/20">
                                                                        ...
                                                                        <td className="px-3 py-2 text-xs text-muted-foreground">{pid || "ΓÇö"}</td>
                                                                        <td className="px-3 py-2 text-right">
                                                                            <Button
                                                                                variant="outline"
                                                                                size="sm"
                                                                                onClick={() => {
                                                                                    ...
                                                                                    goToDashboardProject(pid, "overview")
                                                                                }}
                                                                                disabled={!pid}
                                                                            >
                                                                                Open
                                                                                ...
                                                                                <tr key={method.payment_method_id} className="border-b last:border-b-0">
                                                                                    ...
                                                                                    <td className="px-3 py-2">
                                                                                        {method.card_brand && method.card_last4
                                                                                            ? `${method.card_brand.toUpperCase()} ΓÇóΓÇóΓÇóΓÇó ${method.card_last4}`
                                                                                            : 'ΓÇö'}
                                                                                    </td>
                                                                                    <td className="px-3 py-2">
                                                                                    ...
                                                                                    <td className="px-3 py-2">{method.is_default ? 'Default' : 'ΓÇö'}</td>
                                                                                </tr>
                                                                            ))}
                                                                        </tbody>
                                                                    </table>
                                                                        ...
                                                                        <tbody>
                                                                            {billingChangeHistory.map((change) => (
                                                                                <tr key={change.change_id || change.id} className="border-b last:border-b-0">
                                                                                    ...
                                                                                    <td className="px-3 py-2">{change.change_type || 'ΓÇö'}</td>
                                                                                    <td className="px-3 py-2">{change.from_plan || 'ΓÇö'}</td>
                                                                                    <td className="px-3 py-2">{change.to_plan || 'ΓÇö'}</td>
                                                                                    <td className="px-3 py-2">{change.prorated_amount ? formatPrice(change.prorated_amount) : 'ΓÇö'}</td>
                                                                                </tr>
                                                                            ))}
                                                                        </tbody>
                                                                    </table>
                                                ...
                                                <div className="flex flex-1 flex-col gap-2 sm:flex-row sm:items-center">
                                                    <div className="relative w-full sm:max-w-sm">
                                                        ...
                                                        <Input
                                                            ...
                                                            placeholder="Search message/platform/app versionΓÇª"
                                                            className="pl-8"
                                                        />
                                                    </div>
